...
The bonds and bridges are all OVS ones, XCP-ng does not use linux bridges or bonds, for example, a `bond0` device won't appear in `ip link`, it only exists as a port of an OVS bridge. To have an ip used for it, it will have to be set on the matching bridge interface.
...
The core concepts are introduced on the [Networking page](../../networking). Here, we explain their operation and underlying mechanisms. XAPI stores the configuration and relationships for these objects, then configures OVS on each host in the pool.
...
Here’s a quick glossary of main OVS elements (details follow):
...
:::tip
NL stands for netlink, OVS uses almost exclusively netlink to communicate between the kernel and userland, through Generic Netlink commands.
:::
