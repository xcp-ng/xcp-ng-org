...
:::warning
When creating a VLAN network, setting its MTU higher than the underlying physical Network can cause packets to be dropped by the OVS bridge. See [MTU and VLAN interaction](#mtu-and-vlan-interaction) below for details.
:::
...
When you create a VLAN network in Xen Orchestra, it will share the same bridge as the underlying network.

If the VLAN network is configured with a higher MTU than the underlying network it rides on, packets sent from a that VLAN will be dropped, not fragmented, as this is Layer 2 territory. The bridge cannot forward a frame that is larger than the egress port's MTU.
