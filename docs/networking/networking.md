...
## :mortar_board: Concepts {#concepts}
...
If the VLAN-enabled network is configured with a higher MTU than the underlying network it rides on, packets sent from that VLAN will be dropped, not fragmented, as this is Layer 2 territory. The bridge cannot forward a frame that is larger than the egress port's MTU.
...
## :dart: Dedicated storage NIC {#dedicated-storage-nic}
...
## :compass: Management interface {#management-interface}
...
## :vertical_traffic_light: Limit a VM's network bandwidth (QoS) {#limit-a-vms-network-bandwidth-qos}
...
## :name_badge: Restrict what a VM can send (switch port locking) {#switch-port-locking}
...
## :articulated_lorry: NBD backup network {#nbd-backup-network}
...
## :globe_with_meridians: Manage physical NICs {#manage-physical-nics}
...
## :wheel: SDN controller {#sdn-controller}
...
## :busstop: Static routes {#static-routes}
...
## :spider_web: Full mesh network {#full-mesh-network}
...
## :mag_right: DNS Search Domains {#dns-search-domains}
...
## :construction_worker_man: Network Troubleshooting {#network-troubleshooting}
