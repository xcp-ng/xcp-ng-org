...
## :building_construction: Create a pool {#create-a-pool}
...
## :door: Remove a host from a pool {#remove-a-host-from-a-pool}
...
## :crown: Change the pool coordinator {#change-the-pool-coordinator}
...
## :wrench: Maintenance mode {#maintenance-mode}
...
## :key: Root password {#root-password}
...
## :alarm_clock: Time synchronization (NTP) {#time-synchronization-ntp}
...
## :electric_plug: Remote host power-on {#remote-host-power-on}
...
## :handshake: Pool-wide settings {#pool-wide-settings}
...
## :stethoscope: Pool database backup {#pool-database-backup}
