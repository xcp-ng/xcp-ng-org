...
## :radioactive: Release Notes & Known Issues {#release-notes--known-issues}
...
## :cd: Upgrade via installation ISO (recommended) {#upgrade-via-installation-iso-recommended}
...
## :technologist: From command line {#from-command-line}
...
## :regional_indicator_x: Upgrade from XenServer {#upgrade-from-xenserver}
...
## :older_man: Migrate VMs from older XenServer/XCP-ng {#migrate-vms-from-older-xenserverxcp-ng}
...
## :biohazard: Handling alternate drivers or kernel {#handling-alternate-drivers-or-kernel}
