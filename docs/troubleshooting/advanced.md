...
## :thread: Serial console access {#serial-console-access}
...
## :gear: Xen boot options {#xen-boot-options}
...
## :mag: Reading the hypervisor state {#reading-the-hypervisor-state}
...
## :boom: Host crashes and crash dumps {#host-crashes-and-crash-dumps}
...
## :hiking_boot: When the host doesn't boot {#when-the-host-doesnt-boot}
...
## :sos: Produce a status report {#produce-a-status-report}
