...
## :scroll: General log {#general-log}
...
## :satellite: XAPI's log {#xapis-log}

`/var/log/xensource.log`

Contains the output of the XAPI toolstack.

## :minidisc: Storage related (eg. coalescing snapshots) {#storage-related-eg-coalescing-snapshots}

`/var/log/SMlog`

Contains the output of the storage manager.

## :penguin: Kernel messages {#kernel-messages}
...
## :boom: Kernel crash logs {#kernel-crash-logs}

In case of a host crash, if it is kernel-related, you should find logs in `/var/crash`

## :clipboard: Produce a status report {#produce-a-status-report}
...
## :window: XCP-ng Center {#xcp-ng-center}
...
## :desktop_computer: Windows VM {#windows-vm}

### (PV-)Driver install log
`C:\Windows\INF\setupapi.dev.log`


## :mag: Useful data for debugging {#useful-data-for-debugging}
