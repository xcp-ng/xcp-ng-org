...
## :snail: Low storage performance on Windows {#low-storage-performance-on-windows}
...
## :hiking_boot: Windows fails to boot (hangs, `INACCESSIBLE_BOOT_DEVICE`) {#windows-fails-to-boot-hangs-inaccessible_boot_device}
...
## :thread: Connecting to guests using serial console {#connecting-to-guests-using-serial-console}
...
## :blue_heart: BSOD 0x3B (SYSTEM_SERVICE_EXCEPTION) or 0x7E (SYSTEM_THREAD_EXCEPTION_NOT_HANDLED) on newer Intel CPUs {#bsod-0x3b-system_service_exception-or-0x7e-system_thread_exception_not_handled-on-newer-intel-cpus}
...
## :boom: BSOD 0x109 (CRITICAL_STRUCTURE_CORRUPTION) after migrating VMs on Intel platforms {#bsod-0x109-critical_structure_corruption-after-migrating-vms-on-intel-platforms}
...
## :recycle: Windows fails to boot after updating {#windows-fails-to-boot-after-updating}
...
## :zipper_mouth_face: Windows Server 2019 crashes on AMD Zen without BSOD {#windows-server-2019-crashes-on-amd-zen-without-bsod}
...
## :floppy_disk: Data drives showing as removable in Windows {#data-drives-showing-as-removable-in-windows}
...
## :no_entry_sign: XenServer VM Tools not upgrading drivers after installation {#xenserver-vm-tools-not-upgrading-drivers-after-installation}
...
## :arrows_counterclockwise: XenServer drivers not receiving updates via Windows Update {#xenserver-drivers-not-receiving-updates-via-windows-update}
...
## :stethoscope: Malfunctioning XenServer PV drivers {#malfunctioning-xenserver-pv-drivers}
...
## :ice_cube: Windows Server 2025 hangs randomly with 0% CPU {#windows-server-2025-hangs-randomly-with-0-cpu}
...
## :brain: How to gather kernel memory dumps for in-depth troubleshooting {#how-to-gather-kernel-memory-dumps-for-in-depth-troubleshooting}
