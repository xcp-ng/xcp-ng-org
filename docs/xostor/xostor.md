...
If you are reading this documentation, we assume that you want to upgrade a XOSTOR pool, i.e. change the version of XCP-ng, for example from 8.2 to 8.3.
For updates that don't change the version numbers (bugfixes, security fixes), see [the update section](#update).

### 1. Prerequisites

- You want to upgrade to an officially released and LTS version of XCP-ng.
...
- The HA must be disabled on your pool.
...
From this point we can proceed to upgrade your XOSTOR pool.
...
1. Update the host following the instructions in [this guide](../installation/upgrade/#-upgrade-via-installation-iso-recommended).
