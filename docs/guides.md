...
4. (After a reboot or host startup) Move VMs back to it if appropriate. There is no need to re-enable the host: it is done automatically when it starts.
...
1. Put the host into maintenance mode from Xen Orchestra. This will disable the host, then evacuate its VMs automatically to other hosts.
   If you prefer to do it from command line, this is equivalent to: `xe host-disable host={hostname} && xe host-evacuate host={hostname}`.
2. Reboot or shutdown
   Can be done from XO, or from command line: `xe host-reboot|host-shutdown host={hostname}`.
3. (After a reboot or host startup) Move VMs back to it if appropriate. There is no need to re-enable the host: it is done automatically when it starts.
